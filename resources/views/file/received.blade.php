            ...
            <div class="flex justify-center items-center float-right">
                <form action="{{route('file.index')}}" method="get" class="flex flex-wrap justify-between md:flex-row rounded-lg overflow-hidden">
                    ...
                    <select name="status" class="border-l border-r border-gray-100 border-t-0 border-b-0 flex-1 text-gray-700 placeholder-gray-400 bg-white appearance-none dark:text-gray-200 focus:outline-none focus:border-gray-100 focus:ring-0 max-w-48">
                        <option value="">All Received Files</option>
                        <option {{request()->has('status') &&  @request()->get('status')== 'in'  ? 'selected':'' }} value="in">Still in Office</option>
                        <option {{request()->has('status') &&  @request()->get('status')== 'out'  ? 'selected':'' }} value="out">Forwarded / Returned</option>
                    </select>
                        ...
                        <tr class="border-b border-gray-200 bg-white text-black hover:bg-gray-100">
                            ...
                            <td class="py-3 px-6 text-center w-48">
                                ...
                                @else
                                    ...
                                    <span class="bg-gray-200 text-gray-600 py-1 px-3 rounded-full block">Under Process</span>
                                    @elseif($file->curr_received_date < date('Y-m-d H:i:s',strtotime('-5 days')))
                                        <span class="bg-red-200 text-red-600 py-1 px-3 rounded-full block">Delayed<br><span class="text-xs">Since: {{\Carbon\Carbon::createFromFormat('Y-m-d H:i:s', $file->curr_received_date)->diffForHumans()}}</span></span>
                                    @else
